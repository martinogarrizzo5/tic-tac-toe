...
const buttonHTML =
  '<button class="btn" id="game-btn" onclick="startGame()">Play Now</button>';
const playAgainButton =
  '<button class="btn" id="play-again-button" onclick="playAgain()">Play Again</button>';
...
function endGame() {
  ...
  if (isGameOver) {
    container!.innerHTML += `<div class="game-over"><h3 class="title">${
      winningPlayer === "X" ? UserWinText : UserLostText
    }</h3>${playAgainButton}</div>"`;
  } else {
